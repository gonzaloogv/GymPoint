    ...
    <>
      <InfoCard variant="default" className="mx-4 mt-4">
        <View className="flex-row items-center mb-3">
          <View
            className="w-14 h-14 rounded-[20px] border justify-center items-center mr-3"
            style={{
              backgroundColor: isDark ? 'rgba(168, 85, 247, 0.22)' : 'rgba(168, 85, 247, 0.16)',
              borderColor: isDark ? 'rgba(168, 85, 247, 0.38)' : 'rgba(168, 85, 247, 0.24)',
            }}
          >
            <Feather name="credit-card" size={20} color={isDark ? '#c084fc' : '#9333ea'} />
          </View>
          <Text className="text-lg font-semibold" style={{ color: isDark ? '#F9FAFB' : '#111827' }}>
            Suscripción
          </Text>
        </View>
        ...
        {subscriptionStatus.isLoading ? (
          ...
          <>
            {/* Active Member Badge */}
            <View className={`rounded-[24px] px-5 py-[18px] mb-4 ${isDark ? 'bg-surface-dark' : 'bg-white'} ${subscriptionStatus.subscription.isExpiringSoon ? 'border-2 border-yellow-500/50' : ''}`}>
              <View className="flex-row items-center mb-3">
                ...
                <View className="flex-1">
                  <Text className="text-xl font-bold" style={{ color: isDark ? '#F9FAFB' : '#111827' }}>
                    Eres socio activo
                  </Text>
              ...
              <View className={`rounded-[16px] p-3 ${isDark ? 'bg-surface-dark/50' : 'bg-white/50'}`}>
                <View className="flex-row justify-between items-center mb-2">
                  <Text className="text-sm font-medium" style={{ color: isDark ? '#9CA3AF' : '#6B7280' }}>
                    Vencimiento
                  </Text>
                  <Text className="text-sm font-bold" style={{ color: isDark ? '#F9FAFB' : '#111827' }}>
            ...
            <TouchableOpacity
              className="bg-red-500 rounded-[24px] px-4 py-3 items-center"
              onPress={() => {
                Alert.alert(
          ...
          <>
            ...
            {subscriptionStatus.canUseTrial ? (
              <TouchableOpacity
                className="bg-blue-500/10 border border-blue-500/30 rounded-[24px] px-4 py-3 mb-3 flex-row items-start"
                onPress={() => {
                  Alert.alert(
              ...
              >
                ...
                <View className="flex-1">
                  <Text className="font-semibold mb-1" style={{ color: isDark ? '#F9FAFB' : '#111827' }}>
              ...
              <View className="bg-gray-500/10 border border-gray-500/30 rounded-[24px] px-4 py-3 mb-3 flex-row items-start">
            ...
            {/* Subscribe Button */}
            <TouchableOpacity
              className="bg-primary rounded-[24px] px-4 py-4 items-center"
      ...
      >
        <View className="flex-1 justify-end" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <View className={`${isDark ? 'bg-background-dark' : 'bg-background'} rounded-t-[28px] px-6 pt-[18px] pb-8`}>
            <Text className="text-2xl font-bold mb-6" style={{ color: isDark ? '#F9FAFB' : '#111827', letterSpacing: -0.2 }}>
              Suscribirse a {gymName}
            </Text>

            <Text className="text-sm mb-3" style={{ color: isDark ? '#9CA3AF' : '#6B7280' }}>
              Seleccioná tu plan:
            </Text>
            <View className="flex-row gap-2 mb-6">
              {(['SEMANAL', 'MENSUAL', 'ANUAL'] as const).map((plan) => (
                <TouchableOpacity
                  key={plan}
                  className={`flex-1 py-3 px-4 rounded-[20px] border-2 ${
                    selectedPlan === plan
                ...
                >
                  <Text
                    className={`text-center font-semibold ${
                      selectedPlan === plan ? 'text-onPrimary' : ''
                    }`}
                    style={selectedPlan !== plan ? { color: isDark ? '#F9FAFB' : '#111827' } : undefined}
                  >
                    {plan}
                  </Text>
                </TouchableOpacity>
            ...
            <Text className="text-sm mb-2" style={{ color: isDark ? '#9CA3AF' : '#6B7280' }}>
              Fecha de vencimiento:
            </Text>
            <TouchableOpacity
              className={`border rounded-[20px] p-4 mb-6 ${isDark ? 'border-border-dark bg-surface-dark' : 'border-border bg-white'}`}
              onPress={() => setShowDatePicker(true)}
            >
              <Text className="text-base font-semibold" style={{ color: isDark ? '#F9FAFB' : '#111827' }}>
            ...
            <View className="flex-row gap-3">
              <TouchableOpacity
                className={`flex-1 py-4 rounded-[20px] ${isDark ? 'bg-surface-dark' : 'bg-gray-200'}`}
                onPress={() => setShowManualSubscribeModal(false)}
              >
                <Text className="text-center font-semibold" style={{ color: isDark ? '#F9FAFB' : '#111827' }}>
                  Cancelar
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                className="flex-1 bg-primary py-4 rounded-[20px]"

...
const listGymSpecialSchedules = async (req, res) => {
  try {
    ...
    const resultado = await service.obtenerHorariosEspecialesPorGimnasio(Number.parseInt(gymId, 10));
    res.json(resultado);
  } catch (err) {
    res.status(404).json({
      error: {
...
const createGymSpecialSchedule = async (req, res) => {
  try {
    ...
    const resultado = await service.crearHorarioEspecial({
      id_gym: Number.parseInt(gymId, 10),
      date,
      opening_time,
      closing_time,
      closed,
...
const updateGymSpecialSchedule = async (req, res) => {
  try {
    ...
    const schedule = await GymSpecialSchedule.findByPk(Number.parseInt(id, 10));

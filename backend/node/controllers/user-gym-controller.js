...
const darAltaEnGimnasio = async (req, res) => {
  try {
    const { id_user, id_gym, plan } = req.body;
    if (!id_user || !id_gym || !plan) {
...
const obtenerGimnasiosActivos = async (req, res) => {
    try {
      const resultado = await userGymService.obtenerGimnasiosActivos(req.params.id_user);
      res.json(resultado);
    } catch (err) {
      res.status(400).json({ error: err.message });
    }

...
const registrarPago = async (req, res) => {
  try {
    const { id_user, id_gym, mount, payment_method, payment_date, status } = req.body;
    if (!id_user || !id_gym || !mount || !payment_method || !payment_date || !status) {
      return res.status(400).json({ error: 'Faltan datos requeridos.' });
    }

    const pago = await service.registrarPago({
      id_user, id_gym, mount, payment_method, payment_date, status
    });
...
const obtenerPagosPorUsuario = async (req, res) => {
  try {
    const pagos = await service.obtenerPagosPorUsuario(req.params.id_user);
    res.json(pagos);
  } catch (err) {
    res.status(400).json({ error: err.message });
  }

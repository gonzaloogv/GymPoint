...
const GymType = require('../models/GymType')(
...
const filtrarGimnasios = async ({ city, type, minPrice, maxPrice }) => {
  ...
  if (minPrice || maxPrice) {
    where.month_price = {};
    if (minPrice) {
      where.month_price[Op.gte] = minPrice;
    }
    if (maxPrice) {
      where.month_price[Op.lte] = maxPrice;
    }
  }
...
const getGymTypes = () => {
  return ['completo', 'crossfit', 'powerlifting', 'femenino', 'boxeo', 'express', 'exclusivo'];
};
